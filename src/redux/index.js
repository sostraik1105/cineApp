...
const reducer = (state = INITIAL_STATE, action) => {
  switch (action.type) {
    ...
    case actions.agregarCarritoDulce:
      ...
      let buscard = state.carrito.find(
        (el) => el.id_dulceria === nuevod.id_dulceria
      );
      return buscard
        ? {
            ...state,
    ...
    case actions.agregarCarritoPeli:
      let nuevop = state.movies.find(
        (p) => p.id_pelicula === parseInt(action.payload)
      );
      let buscarp = state.carrito.find(
        (item) => item.id_pelicula === nuevop.id_pelicula
      );
      return buscarp
        ? {
            ...state,
        ...
        : {
            ...
            carrito: [
              ...state.carrito,
              { ...nuevop, cantidad: 1, precio_venta: 20 },
            ],
          };
    case "FILTRADO":
      return {
